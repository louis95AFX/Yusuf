...
// Get reference to the container from unisex.html
...
function renderProducts(products) {
    ...
    if (products.length === 0) {
        productListElement.innerHTML = '<p style="grid-column: 1 / -1; text-align: center;">No unisex perfumes found.</p>';
        return;
    }
...
/**
 * Fetches only products where the category is 'Unisex' (case-insensitive).
 */
async function fetchUnisexProducts() {
    if (!productListElement) return;
    ...
    productListElement.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 50px 0;">
            ...
            <p>Loading unisex perfumes...</p>
        </div>
    `;

    // CRITICAL: Use the .ilike() filter for case-insensitive matching against 'unisex'
    const { data: products, error } = await supabaseClient
        .from('products')
        .select('*')
        .ilike('category', 'unisex') // <--- Targets the 'unisex' category
        .order('id', { ascending: false });

    if (error) {
        console.error('Error fetching unisex products:', error.message || error);
